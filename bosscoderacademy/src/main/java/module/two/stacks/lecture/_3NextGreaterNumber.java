 ...
 * Next Greater Number:
 * <p>
 * Find the next greater number nearest to current element.
 * <p>
 * 1. Input = [4, 8, 5, 2, 25], Output = [8, 25, 25, 25, -1]
 * 2. Input = [3, 4, 2, 1, 11, 9, 77], Output = [4, 11, 11, 11, 77, 77, -1]
 */
public class _3NextGreaterNumber {
     ...
     * - Now, for each element search in right side and store the first greater element found, at that index.
     * - Once, greater number found just break and stop searching for other numbers.
    ...
    private static int[] approach1(int[] arr) {
        ...
        for (int i = 0; i < n; i++) {
            for (int j = i; j < n; j++) {
                ...
                // no greater element found for current element
                if (j == n - 1) { // true for last element or greatest in mid
                    res[i] = -1;
                }
            }
        }
     ...
     * - Then, checking inside while for top stack element, if it is less than current element just pop it.
